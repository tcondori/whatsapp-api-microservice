...
class ConversationRepository(BaseRepository):
    ...
    def update_context(self, phone_number: str, state: str = None, data: dict = None) -> bool:
        ...
        try:
            ...
            if data:
                current_data = context.conversation_data or {}
                current_data.update(data)
                updates['conversation_data'] = current_data
            
            result = self.update(context.id, **updates)
            if result:
                self.logger.info(f"Contexto actualizado para {phone_number} - Estado: {state}")
                return True
